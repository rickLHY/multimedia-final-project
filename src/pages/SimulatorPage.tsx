...
export default function SimulatorPage({ mode, multimediaMode }: Props) {
  ...
  const stepNarrationText = useMemo(() => {
    if (isBuy) {
      if (step === 1) return `
融資，就是向券商借錢買股票，放大你的投資部位。
舉個例子：你有五萬元自備款，以百分之五十的初始保證金融資，就能買進十萬元的股票，等於兩倍槓桿。
資金結構很簡單：股票市值，等於借款加上帳戶權益。
帳戶權益就是你真正擁有的部分，股票市值減掉借款就是你的權益。
維持保證金比率，簡稱 MMR，是帳戶權益除以股票市值的最低底線，預設百分之四十。
一旦股價下跌，讓這個比率低於 MMR，券商就會要求你補錢，也就是追繳。
若補不上，券商會強制賣掉你的持股，俗稱斷頭。
請設定股數和買入單價，調整 MMR，然後點選「開啟動態沙盒」進入模擬。
      `.trim();

      return `
現在進入融資動態模擬。向左拖動滑桿，就是模擬股價持續下跌。
注意比例圖裡，藍色的借款完全不動，只有綠色的帳戶權益在縮水。
這就是槓桿最殘酷的地方：虧損全在你這邊，借款一分不少。
當帳戶權益除以市值，跌破維持保證金比率，警報就會響起，追繳通知啟動。
碰到追繳有三條路：第一，補交現金；第二，賣股減少借款；第三，如果都不做，券商強制平倉，也就是斷頭。
試試看，把滑桿拉到最左邊，看看槓桿崩潰的感覺。
      `.trim();
    }

    if (step === 1) return `
融券，就是借股票來賣，預期股價會跌，之後再低價買回還給券商，賺取中間差價。
融券的資金結構跟融資剛好相反。
賣出借來股票的現金，加上你自己繳的保證金，合起來叫做「總保證金帳戶餘額」，這個數字是固定的。
你的融券權益，等於這個固定餘額，減去當下買回股票所需的市值。
所以股價漲得越高，回補成本越大，你的權益就越少。
維持保證金比率一旦跌破，就會觸發融券追繳。
融券最可怕的風險叫做軋空，Short Squeeze。
大量放空者同時被迫回補，推動股價暴漲，形成恐慌性上漲，損失可以在很短的時間內變得非常巨大。
放空理論上的最大虧損是無限的，因為股價沒有上限。
    `.trim();

    return `
現在進入融券動態模擬。向右拖動滑桿，就是模擬股價持續上漲，也就是軋空的走勢。
注意比例圖裡，橘色的回補成本不斷往右擴大，把綠色的融券權益往角落擠。
這就是融券的關鍵：你的資產總額固定不動，但負債隨股價爬升而增加。
當保證金成數跌破維持比率，追繳警報立刻啟動。
碰到融券追繳，你必須立刻補交現金，或者買回股票回補。
如果拖著不動，券商會在高點幫你強制買回，那個損失是真實的、永久的。
把滑桿推到最右邊，感受一下軋空是什麼感覺。
    `.trim();
  }, [isBuy, step]);
